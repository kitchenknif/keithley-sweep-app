...
class Keithley:
    ...
    def factory(model, port):
        ...
        assert 0, "Bad identification string: " + model
...
class Keithley_24XX(Keithley):
    ...
    # Factory Sweep Thing
    #
    def doLegacySweep(self, startV=0.0, endV=3.0, numberOfPoints=100, remoteSensing=True,
                      logSteps=False, autorange=True, measureRange=1e-3, limit=1.0, pulseSweep=False,
                      pulseWidth=1e-3, pulseDelay=1e0):
        if self.port.closed:
            return False
        ...
        if autorange and not pulseSweep:
            ...
            s = ":SENS:CURR:RANGE " + str(measureRange) + ";\n"
    ...
    def armDCMeasurements(self, remoteSensing=True, autorange=True, measureRange=1e-3, limit=1.0):
        if self.port.closed:
            return False
    ...
    #
    def armPulseMeasurements(self, remoteSensing=True, measureRange=1e-3, limit=1.0, pulseWidth=1e-3, pulseDelay=1e0):
        if self.port.closed:
            return False
        ...
        s = ":SENS:CURR:PROT " + str(limit) + "; \n"
        self.port.write(s.encode('ascii'))
        s = ":SENS:CURR:RANG " + str(measureRange) + "; \n"
        self.port.write(s.encode('ascii'))
        self.port.write(b":FORM:ELEM VOLT,CURR; \n")
...
class Keithley_2635a(Keithley):
    ...
    def doLegacySweep(self, startV=0.0, endV=3.0, numberOfPoints=100, remoteSensing=True, logSteps=False,
                      autorange=True, measureRange=1e-3, limit=1.0, pulseSweep=False, pulseWidth=1e-3, pulseDelay=1e0):
        if self.port.closed:
            return False
        if not self.supportPulse and pulseSweep:
            return False
        ...
        if autorange and not pulseSweep:
            ...
            s = "smua.measure.autorangev = smua.AUTORANGE_OFF;" \
                + "smua.measure.autorangei = smua.AUTORANGE_OFF; " \
                + "smua.measure.rangei = " + str(measureRange) + ";\n"
            s += "smua.source.autorangei = smua.AUTORANGE_OFF;" \
                 + "smua.source.autorangev = smua.AUTORANGE_OFF; " \
                 + "smua.source.rangev = " + str(measureRange) + ";\n"
        ...
        if not pulseSweep:
            ...
            s += "(smua,0," + str(startV) + ', ' + str(endV) + ', ' + str(limit) + ', ' \
                 + str(pulseWidth) + ', ' + str(pulseDelay) + ', ' + str(numberOfPoints) + "smua.nvbuffer1, 1)"
